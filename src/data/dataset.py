...
class PairedBioassayDataset(Dataset):
    """
    Dataset that produces paired samples for Triple-Check training.

    Each sample returns ``(img_t1, img_u1, img_t2, img_u2)`` where:
      - ``img_t1`` / ``img_u1`` are treated / untreated images from plate 1
      - ``img_t2`` / ``img_u2`` are treated / untreated images from plate 2

    If ``num_untreated_samples > 1`` the untreated tensors have an extra leading
    dimension ``(N, C, H, W)`` so the trainer can average them.

    Wraps :class:`CompoundPlateDataset` internally for metadata I/O.
    """
    ...
    def __init__(
        ...
        """
        Args:
            root_dir: Root directory with plate images and metadata.
            metadata_file: Name of metadata JSON file.
            transform: Image transforms to apply.
            num_untreated_samples: Number of untreated (control) images to
                sample per plate for each pair.  If > 1 the returned
                untreated tensor has shape ``(N, C, H, W)``; otherwise
                ``(C, H, W)``.
        """
        self.root_dir = Path(root_dir)
        self.transform = transform
        ...
        compounds = raw.get("compounds", raw) if isinstance(raw, dict) else raw

        # Build a flat list of (compound_entry, plate_a, plate_b) triples
        # so that __len__ is deterministic.
        self._pairs: List[Tuple[dict, str, str]] = []
        for compound in compounds:
            plate_keys = [k for k in compound if k not in ("id", "Compound")
                          and isinstance(compound[k], dict)]
            # Need at least two plates for a pair
            if len(plate_keys) < 2:
                continue
            for i in range(len(plate_keys)):
                for j in range(i + 1, len(plate_keys)):
                    self._pairs.append((compound, plate_keys[i], plate_keys[j]))

        if len(self._pairs) == 0:
            raise RuntimeError(
                "No valid plate pairs found.  Each compound needs at least "
                "two plates with both treated and control samples."
            )
    ...
    def __getitem__(self, idx: int):
        compound, plate_a, plate_b = self._pairs[idx]

        img_t1 = self._sample_image(compound[plate_a]["treated"])
        img_u1 = self._sample_images(compound[plate_a]["control"],
                                     self.num_untreated_samples)

        img_t2 = self._sample_image(compound[plate_b]["treated"])
        img_u2 = self._sample_images(compound[plate_b]["control"],
                                     self.num_untreated_samples)
    ...
    def _load_and_transform(self, rel_path: str) -> torch.Tensor:
    ...
    def _sample_image(self, paths: List[str]) -> torch.Tensor:
        """Pick one random image and return ``(C, H, W)``."""
        path = random.choice(paths)
        return self._load_and_transform(path)

    def _sample_images(self, paths: List[str], n: int) -> torch.Tensor:
        """Pick *n* random images.

        Returns ``(C, H, W)`` when ``n == 1``, else ``(N, C, H, W)``.
        """
        chosen = random.choices(paths, k=n)
        tensors = [self._load_and_transform(p) for p in chosen]
        if n == 1:
            return tensors[0]
...
create_paired_metadata = auto_create_compound_plate_metadata
